...
const badgeDeltaVariants = cva("inline-flex items-center text-tremor-label font-semibold", {
  ...
  compoundVariants: [
    ...
    },
    {
      deltaType: "decrease",
      variant: "solid",
      className: "bg-red-100 text-red-800 dark:bg-red-400/20 dark:text-red-500",
    },
    {
      deltaType: "neutral",
      variant: "solid",
...
export function BadgeDelta({
      ...
      <span className={cn(badgeDeltaVariants({ variant, className }))} {...props}>
        <span
          className={cn(
            ...
            d === "increase" && "text-emerald-700 dark:text-emerald-500",
            d === "decrease" && "text-red-700 dark:text-red-500",
            d === "neutral" &&
        ...
        <span
          className={cn(
            ...
            d === "increase" && "bg-emerald-100 dark:bg-emerald-400/10",
            d === "decrease" && "bg-red-100 dark:bg-red-400/10",
            d === "neutral" &&
              "bg-tremor-background-subtle dark:bg-dark-tremor-background-subtle"
          )}
        >
          <DeltaIcon deltaType={d} iconStyle="line" />
        </span>
      </span>
    );
  }
  ...
  return (
    <span className={cn(badgeDeltaVariants({ variant, deltaType: d, className }))} {...props}>
      <DeltaIcon deltaType={d} iconStyle={i} />
      {value}
    </span>
  );
}
